...
class Hint:
    ...
    def extract_hint(self, text):
        ...
        extracted_hint = text[start_index:end_index].strip()

        explanation_start_index = text.find(explanation_start_tag)
        if explanation_start_index == -1:
            return extracted_hint, ""
    
        explanation_start_index += len(explanation_start_tag)
        explanation_end_index = text.find(explanation_end_tag, explanation_start_index)
        if explanation_end_index == -1:
            explanation_end_index = len(text)
    
        extracted_explanation = text[explanation_start_index:explanation_end_index].strip()
    ...
    def generate_hint(self, problem_data, buggy_program, testcases):  
        ...
        hint, explanation = self.extract_hint(generated_response)
