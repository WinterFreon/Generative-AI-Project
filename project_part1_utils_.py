...
class Distance:
    ...
    def get_edit_distance(self, source1: str, source2: str) -> int:
        try:
            tokens1 = self.program_to_essential_tokens(source1)
            tokens2 = self.program_to_essential_tokens(source2)
            distance = self.token_edit_distance(tokens1, tokens2)
...
def calculate_pass_rate(candidate_code, testcases):
        passed_tests = 0
        ...
        for testcase in testcases:
            try:
                ...
                actual_output = run_code(candidate_code, input_data)

                if actual_output == expected_output:
                    passed_tests += 1
            except Exception as e:
                print(f"Test case failed: {testcase}, Error: {e}")
        return (passed_tests / total_tests) * 100 if total_tests > 0 else 0

def run_code(code, input_data):
        exec_globals = {}
        exec(code, exec_globals)
        return eval(f"main({input_data})", exec_globals)
